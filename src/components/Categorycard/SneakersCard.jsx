...
const SneakersCard = ({ sneaker, onAddToCart }) => {
  return (
    <div className="flex flex-col justify-center items-start p-2 bg-white">
      <div className="relative">
        <a href={`/product-details/${sneaker.id}`}>
          <img
            className="lg:block hidden max-w-lg"
            src={sneaker.image.url}
            alt="headphones"
          />
          <img className="lg:hidden" src={sneaker.image.url} alt="headphones" />
        </a>
        <button
          onClick={() => onAddToCart(sneaker.id, 1)}
